...
class LoRAExperimentConfig:
    ...
    batch_size: int = 8                  # 每批大小（GPU 不够就减小，如 4）
    ...
    # ---- LoRA 参数 ----
    lora_r: int = 8                      # LoRA rank（8 是常见起步值）
    lora_alpha: int = 16                 # LoRA alpha（一般是 r 的 2 倍）
    ...
    # ---- SupCon（监督对比损失）参数 ----
    # 新增：融合 SupCon Loss 的创新点。
    # supcon_lambda = 0 等价于纯 CE loss（baseline）
    # 论文中建议测试 λ ∈ {0, 0.1, 0.3, 0.5, 1.0}
