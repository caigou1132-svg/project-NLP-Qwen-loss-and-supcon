...
from code.config import LoRAExperimentConfig, RESULTS_DIR, resolve_model_path
from code.supcon_loss import SupConLoss, pool_hidden_states
...
def train_one_epoch(
    ...
    for batch in tqdm(train_loader, desc="Training", leave=False):
        ...
        if supcon_lambda > 0 and supcon_criterion is not None:
            ...
            features = pool_hidden_states(hidden_states, attention_mask).float()
...
def save_experiment_result(result: dict, config: LoRAExperimentConfig):
    ...
    path = os.path.join(config.output_dir, f"result_{safe_name}{fs_tag}{r_tag}{supcon_tag}{seed_tag}.json")
